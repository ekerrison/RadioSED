...
class RaiSEDModel():
    ...
    #initial function
    def __init__(self, data, model_type: str, model_func: FunctionType, src_name: str, fit_min = 1.0e7, fit_max = 1e11, fit_errs = True, new_flag = False, output_dir = 'output',\
    plot_colour = 'darkred', plot_linestyle = 'solid', savestr_end = ''):
        #self.data = data
    ...
    def set_likelihood(self):
        self.likelihood = bilby.likelihood.GaussianLikelihood(x=self.freq,y=self.flux, func=self.__SED_func__, sigma=self.ferr)
        #self.likelihood = likelihood
        return

    #creating the sampler
    def set_sampler(self, sampler_type = 'dynamic_dynesty', nlive = 512, maxbatch = 1, n_effective = None, use_stop = False, npool = None, check_point = True, overwrite = False): #mabatch 1
        self.sampler_label = "{}_{}_{}".format(self.src_name, self.model_type, self.savestr_end) #no suffix to label
    ...
    def run_sampler(self):
        if self.sampler_type == 'dynesty':
            self.result = bilby.run_sampler(likelihood=self.likelihood, priors=self.prior, sampler=self.sampler_type, nlive=self.nlive,
                                   label=self.sampler_label, outdir=self.output_dir, npool = self.npool, check_point = self.check_point, clean = self.overwrite,
                                   n_effective = self.n_effective)

        elif self.sampler_type == 'dynamic_dynesty':
            #print('USING DYNAMIC DYNESTY FROM RAISED')
            self.result = bilby.run_sampler(likelihood=self.likelihood, priors=self.prior, sampler=self.sampler_type, nlive=self.nlive, maxbatch = self.maxbatch, use_stop = self.use_stop,
                                   label=self.sampler_label, outdir=self.output_dir, npool = self.npool, check_point = self.check_point,
                                   nlive_init=self.nlive, nestcheck=True, clean = self.overwrite, n_effective = self.n_effective, sample='rwalk')
    ...
    def get_bic(self):
        ...
        self.chi_squared = lambda params: np.sum(np.power((self.__SED_func__(self.freq, *params)-self.flux)/self.ferr, 2))

        # Calculate best fit chisquared
        self.fit_params = [self.result.get_one_dimensional_median_and_error_bar(x).median for x in self.priorkeys]
    ...
    def get_log10z(self):
        ...
        try:
            ...
            raise AttributeError('{} has no result object, have you called run_sampler() yet?'.format(self.model_type))
    ...
    def get_best_fit_params(self):
        if not hasattr(self, 'result'):
            raise AttributeError('Cannot get best fit before running the sampler! Try calling run_sampler() first.')
    ...
    def get_fit_range_funcs(self, num = 25):
        ...
        self.randidx = np.random.randint(0, self.result.posterior[self.priorkeys[0]].values.shape[0], size = (num,1))
        self.posterior_list = [np.array([self.result.posterior[x].values[self.randidx]]) for x in self.priorkeys]
        self.best_fit_range =  self.__SED_func__(self.dummy_freq.reshape(1,-1), *self.posterior_list)
    ...
    def get_param_medians_errors(self):
        ...
        self.fit_param_ranges = [self.result.get_one_dimensional_median_and_error_bar(x) for x in self.priorkeys]
