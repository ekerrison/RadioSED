...
class RadioSEDGPModel(RadioSEDModel):
    ...
    def __init__(self, george_model, george_model_defaults, **kwargs):
        ...
        self.gp_params = 2
        self.md = (
            len(signature(self.__SED_func__).parameters) - 1 + self.gp_params
        )  # additional 2 for gp kernel
    ...
    def get_bic(self):
        ...
        self.final_kernel = np.exp(
            self.fit_params_noise[0]
        ) * george.kernels.Matern32Kernel(
            np.exp(self.fit_params_noise[1]), block=(73, 230)
        )
    ...
    def get_best_fit_func(self):
        if not hasattr(self, "fit_params_func"):
            self.fit_params_func = [
                self.result.get_one_dimensional_median_and_error_bar(x).median
                for x in self.priorkeys[: -self.gp_params]
            ]
        ...
        self.best_fit_function = self.__SED_func__(
            self.dummy_freq, *self.fit_params_func
        )
    ...
    def get_fit_range_funcs(self):
        ...
        self.randidx = np.random.randint(
            0, self.result.posterior[self.priorkeys[0]].values.shape[0], size=(25, 1)
        )
        self.posterior_list = [
            np.array([self.result.posterior[x].values[self.randidx]])
            for x in self.priorkeys[: -self.gp_params]
        ]
        self.fit_param_ranges = self.__SED_func__(
