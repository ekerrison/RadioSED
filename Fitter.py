...
class Fitter():
    ...
    def __init__(self, data: pd.DataFrame = pd.DataFrame(), peak_data: pd.DataFrame = pd.DataFrame(), \
        model_list = 'all', name: str = None, lower_freq = 1e7, upper_freq = 1e11, output_dir = 'output', \
        overwrite = False):
        '''
    ...
    def run_single_model(self, has_GLEAM = False, model_type = None, model_func = None, \
        plot_colour = 'g', prior_obj = None, sampler_type = 'dynamic_dynesty', george_model = None, \
        ...
        if has_GLEAM:
            fit = RaiSEDGPModel(data = self.data, model_type = model_type, \
                fit_min = self.fit_min, fit_max = self.fit_max, \
                model_func = model_func, src_name = self.name, plot_colour = plot_colour, \
                output_dir = self.output_dir, george_model = george_model, \
                george_model_defaults = george_model_defaults)
        else:
            fit = RaiSEDModel(data = self.data, model_type = model_type, \
                fit_min = self.fit_min, fit_max = self.fit_max, \
                model_func = model_func, src_name = self.name, plot_colour = plot_colour, \
                output_dir = self.output_dir)
        fit.setup_sampler(prior = prior_obj, sampler_type = sampler_type, maxbatch =self.maxbatch, use_stop = False, \
            n_effective = self.n_effective, overwrite = self.overwrite)
        fit.run_sampler()
        return fit
    ...
    def analyse_fits(self, result_array):
        ...
        if 'retrig' in result_array[0].model_type or 'orienti' in result_array[0].model_type:
            ...
            best_model_info = helper.get_retrig_intervals(result_array[0], SED_func = result_array[0].__SED_func__, \
                gp = result_array[0].gp, freq_min = self.lower_freq, freq_max = self.upper_freq, min_obs_freq = self.min_obs_freq, \
            ...
            best_model_info = helper.get_orienti_intervals(result_array[0], SED_func = result_array[0].__SED_func__, \
                gp = result_array[0].gp, freq_min = self.lower_freq, freq_max = self.upper_freq, min_obs_freq = self.min_obs_freq, \
                max_obs_freq = self.max_obs_freq)
