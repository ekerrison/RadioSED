...
class Plotter():
    ...
    def __init__(self, data: pd.DataFrame = None, peak_data: pd.DataFrame = None,\
         result_array = None, name: str = None,\
        plotpath = 'output/model_plots/', z: float = None, lower_freq = 1e7, \
        upper_freq = 1e11, savestr_end = ''):
        self.result_array = self.update_results(result_array)
        if data is not None:
            self.data, self.peak_data = self.update_data(data, peak_data, name, savestr_end)
    ...
    def update_data(self, data: pd.DataFrame, peak_data: pd.DataFrame, name: str, savestr_end: str):
        ...
        if self.data['Flux Density (Jy)'].isnull().values.any():
            raise ValueError("Cannot fit an SED with null flux density values, please check data inputs!")
        elif self.data['Uncertainty'].isnull().values.any():
            raise ValueError("Cannot fit an SED with null flux uncertainty values, please check data inputs!")
        ...
        self.plot_freqs = self.data.loc[(self.data["Frequency (Hz)"] <= self.fit_max) & (self.data["Frequency (Hz)"] >= self.fit_min), 'Frequency (Hz)']/1e6
        self.plot_fluxes  = self.data.loc[(self.data["Frequency (Hz)"] <= self.fit_max) & (self.data["Frequency (Hz)"] >= self.fit_min), 'Flux Density (Jy)']
        self.plot_errs = self.data.loc[(self.data["Frequency (Hz)"] <= self.fit_max) & (self.data["Frequency (Hz)"] >= self.fit_min), 'Uncertainty']
        self.plot_epochs = self.data.loc[(self.data["Frequency (Hz)"] <= self.fit_max) & (self.data["Frequency (Hz)"] >= self.fit_min),'Refcode'].apply(lambda x: int(x[0:4]))

        #plot peak photometry if it exists
        if peak_data is not None:
            self.plot_peak_freqs = self.peak_data.loc[(self.peak_data["Frequency (Hz)"] <= self.fit_max) & (self.peak_data["Frequency (Hz)"] >= self.fit_min), 'Frequency (Hz)']/1e6
            self.plot_peak_fluxes  = self.peak_data.loc[(self.peak_data["Frequency (Hz)"] <= self.fit_max) & (self.peak_data["Frequency (Hz)"] >= self.fit_min), 'Flux Density (Jy)']
            self.plot_peak_errs = self.peak_data.loc[(self.peak_data["Frequency (Hz)"] <= self.fit_max) & (self.peak_data["Frequency (Hz)"] >= self.fit_min), 'Uncertainty']
    ...
    def init_plot(self):
        ...
        xx = 10 ** (np.linspace(np.log10(self.fit_min/1e6), np.log10(self.fit_max/1e6), 100) )
    ...
    def add_data(self, fig, ax):
        ...
        if self.peak_data is not None:
            ax.errorbar(self.plot_peak_freqs,self.plot_peak_fluxes,yerr=self.plot_peak_errs,linewidth=1,fmt='x', color = 'darkgray', markersize = 5, zorder = 500)

        #plotting integrated flux densities
        main_pts, main_caps, main_barlinecols = ax.errorbar(self.plot_freqs,self.plot_fluxes,yerr=self.plot_errs,linewidth=1,fmt='o', color = 'k', markersize = 2, zorder = 1000)
    ...
    def plot_all_models(self):
        ...
        for idx in range(len(self.result_array)):
            ...
            ax.plot(xx, curve_to_plot , c = self.result_array[idx].plot_colour, \
                linestyle = self.result_array[idx].plot_linestyle, label = self.result_array[idx].model_type, \
                alpha = alphaplot, lw = lw_plot)
        ...
        if 'gp' in self.result_array[0].model_type:
            ...
            self.result_array[0].likelihood.gp.compute(self.data["Frequency (Hz)"]/1e6, self.data["Uncertainty"])
            pred_mean, pred_var = self.result_array[0].likelihood.gp.predict(self.data["Flux Density (Jy)"], xx, return_var=True)
            pred_std = np.sqrt(pred_var)
            ax.plot(xx, pred_mean, c = 'm', lw = 1, label = 'Best GP predicted mean', alpha = 0.6)
            ax.fill_between(
                xx,
                pred_mean + pred_std,
                pred_mean - pred_std,
        ...
        if 'GLEAM' in self.data['Survey quickname'].tolist():
            for idx in dim_order:
                y_pos -= 0.1
                ax.text(0.85, y_pos, '{} '.format(self.result_array[idx].model_type.split('_')[0].title()) + \
            ...
            ax.text(0.85, y_pos - 0.08, 'Best Fit: {}'.format(' '.join(self.result_array[0].model_type.split('_'))), horizontalalignment='center', verticalalignment='center', transform = ax.transAxes, color = 'k', fontsize = 10, weight = 'bold')
            ax.text(0.85, y_pos - 0.13, 'B$_{1,2}$'+ ': {:.3f}'.format(self.result_array[0].get_log10z()[0] - self.result_array[1].get_log10z()[0]), horizontalalignment='center', verticalalignment='center', transform = ax.transAxes, color = 'k', fontsize = 10, weight = 'bold')
        ...
        plt.title(self.name) #title string used to be IAU name / racs comps {n} / peaked : {Y/N}

        plt.savefig(self.plotpath + '_'.join(self.name.split(' ')) + '{}.png'.format(self.savestr_end), bbox_inches = 'tight')
        plt.close()
        return
    ...
    def plot_model_range(self, throw_figure = False, model_idx = 0, fig = None, ax = None):
        ...
        ax.plot(xx, self.plot_curves[model_idx] , c = c_main[model_idx], alpha = 1, label = 'Best model')
        ...
        if len_str_toplot_list >= 4:
            ...
            main_label_x = 0.75
        ax.text(main_label_x, 0.95, 'Best Fit BF: {}'.format(' '.join(self.result_array[0].model_type.split('_'))), horizontalalignment='center', verticalalignment='center', transform = ax.transAxes, color = 'k', fontsize = 10, weight = 'bold')
        ax.text(main_label_x, 0.90, 'log10 Evidence: {:.3f}'.format(self.result_array[0].get_log10z()[0]), horizontalalignment='center', verticalalignment='center', transform = ax.transAxes, color = 'k', fontsize = 10, weight = 'bold')

        #and param strings to the plot
        for str_plot_idx in range(len_str_toplot_list):
            plotstr = str_toplot_list[str_plot_idx]
            if len_str_toplot_list >= 4:
                ax.text(0.55 + 0.3*(str_plot_idx%2), 0.83 - int(str_plot_idx/2)*0.06, plotstr, horizontalalignment='center', verticalalignment='center', transform = ax.transAxes, color = 'k', fontsize = 10)
            else:
                ax.text(0.75, 0.83 - str_plot_idx*0.06, plotstr, horizontalalignment='center', verticalalignment='center', transform = ax.transAxes, color = 'k', fontsize = 10)

        fig.savefig(self.plotpath + '_'.join(self.name.split(' ')) + '_model{}_{}.png'.format(model_idx, self.savestr_end), bbox_inches = 'tight')
        plt.close('all')
        return
    ...
    def plot_best_model(self, throw_figure = False):
        ...
        plt.savefig(self.plotpath + '_'.join(self.name.split(' ')) + '_bestfit{}.png'.format(self.savestr_end), bbox_inches = 'tight')
        plt.close()
        return
    ...
    def plot_epoch(self):
        ...
        #epoch
        ax.errorbar(self.plot_freqs,self.plot_fluxes,yerr=self.plot_errs,linewidth=1,fmt='none', color = 'k', markersize = 2)
        cdata = ax.scatter(self.plot_freqs,self.plot_fluxes,s = 20, c = self.plot_epochs, cmap = 'viridis', zorder = 1000)
        fig.colorbar(cdata, label = 'epoch')

        plt.savefig(self.plotpath + '_'.join(self.name.split(' ')) + '_bestfit{}_epoch.png'.format(self.savestr_end), bbox_inches = 'tight')
        plt.close()
        return
    ...
    def plot_survey(self):
        ...
        #error bars
        plt.errorbar(self.plot_freqs,self.plot_fluxes,yerr=self.plot_errs,linewidth=1,fmt='none', color = 'k', markersize = 2)
        #loop through surveys!
        colour_list = ['royalblue', 'navy', 'deepskyblue', 'teal', 'steelblue', 'lightsteelblue', 'darkslategray', 'blue', 'r', 'g', 'c', 'm']
        markerlist = ['X', 'o', '+', '^', '*', '1', 'P', 'd', 'X', 'o', '+']
        c_idx = 0
        for surv_name in self.data['Survey quickname'].unique().tolist():
            plot_freqs_s = self.data.loc[self.data['Survey quickname'] == surv_name, 'Frequency (Hz)']/1e6
            plot_fluxes_s = self.data.loc[self.data['Survey quickname'] == surv_name, 'Flux Density (Jy)']
            ax.scatter(plot_freqs_s,plot_fluxes_s,s = 30, c = colour_list[c_idx%len(colour_list)], marker = markerlist[c_idx%len(markerlist)], zorder = 1000, label = surv_name)
            c_idx += 1
        plt.legend(fontsize = 10)
        
        plt.savefig(self.plotpath + '_'.join(self.name.split(' ')) + '_bestfit{}_survey.png'.format(self.savestr_end), bbox_inches = 'tight')
        #plt.show()
        plt.close()
        return
    ...
    def plot_publication(self):
        ...
        #plt.grid(True)
        #plt.title(obj_name)
        ax.text(0.7, 0.92, self.name, horizontalalignment='center', verticalalignment='center', transform = ax.transAxes)

        plt.savefig(self.plotpath + '_'.join(self.name.split(' ')) + '_bestfit{}_pub.pdf'.format(self.savestr_end), bbox_inches = 'tight')
        #plt.show()
        plt.close()
        return
    ...
    def plot_restframe(self, z = 0):
        ...
        if self.peak_data is not None:
            ax.errorbar(rest_plot_peak_freqs,self.plot_peak_fluxes,yerr=self.plot_peak_errs,linewidth=1,fmt='x', color = 'darkgray', markersize = 5)

        #plot all flux data points
        ax.errorbar(rest_plot_freqs,self.plot_fluxes,yerr=self.plot_errs,linewidth=1,fmt='o', color = 'k', markersize = 2)
        ...
        #plt.grid(True)
        #plt.title(obj_name)
        ax.text(0.7, 0.92, obj_name, horizontalalignment='center', verticalalignment='center', transform = ax.transAxes)
        ax.xlim(0.5*min(rest_plot_freqs),2*max(rest_plot_freqs))
        
        plt.savefig(self.plotpath + '_'.join(self.name.split(' ')) + '_bestfit{}_restframe.pdf'.format(self.savestr_end), bbox_inches = 'tight')
        plt.close()
        return
