...
class SEDPriors():
    ...
    def __add_gp_prior__(self, prior_obj : bilby.core.prior.PriorDict):
        ...
        prior_obj["kernel:k1:log_constant"] = bilby.prior.Uniform(minimum=-15, maximum=-0.5, name="log_A", latex_label=r"$\ln A$")
        ...
        prior_obj["kernel:k2:metric:log_M_0_0"] = bilby.prior.LogUniform(minimum=1.6, maximum=3.4, name="log_M_0_0", latex_label=r"$\ln M_{00}$")
    ...
    def linear_prior(self, prefix = ''):
        ...
        self.linear_prior_dict[prefix + 'C'] = bilby.prior.LogUniform(minimum = 1e-4, maximum = 1e4)

        #prior for alpha (spectral index) - uniform between -3 and 3
        self.linear_prior_dict[prefix + 'alpha'] = bilby.prior.Uniform(minimum = -4, maximum = 4)
    ...
    def orienti_prior(self, prefix = ''):
        ...
        self.orienti_prior_dict[prefix + "a"] = bilby.prior.Uniform(-100, -1, "a")#-100 -1e-2
        ...
        self.orienti_prior_dict[prefix + "c"] = bilby.prior.TruncatedGaussian(mu = -1, sigma = 5, name = "c", maximum = 0, minimum = -1e12) #-10, 1
    ...
    def snellen_prior(self, prefix = ''):
        ...
        self.snellen_prior_dict[prefix + 'a'] = bilby.prior.LogUniform(minimum = 50, maximum = 50e3)

        #prior for b (peak flux) - from ~50mJy to 10Jy (50e-3 - 10) so LogUniform again
        self.snellen_prior_dict[prefix + 'b'] = bilby.prior.LogUniform(minimum = 30e-3, maximum = 15)

        #prior for c (alpha_thick) - try uniform [0,3] #adjusted to 4 to allow for extreme sources as in fig. 24 of Callingham+2017
        self.snellen_prior_dict[prefix + 'c'] = bilby.prior.Uniform(minimum = 0, maximum = 4)

        #prior for d (alpha_thin) - try uniform [-3, 0] #adjusted to be even with alpha_thick
        self.snellen_prior_dict[prefix + 'd'] = bilby.prior.Uniform(minimum = -4, maximum = 0)
    ...
    def retrig_prior(self, prefix = ''):
        ...
        #prior for a (peak freq) - from ~50MHz to 500 GHz (50-500e3) so we want LogUniform
        self.retrig_prior_dict[prefix + 'a'] = bilby.prior.LogUniform(minimum = 50, maximum = 50e3)

        #prior for b (peak flux) - from ~50mJy to 10Jy (50e-3 - 10) so LogUniform again
        self.retrig_prior_dict[prefix + 'b'] = bilby.prior.LogUniform(minimum = 30e-3, maximum = 15)

        #prior for c (alpha_thick) - try uniform [0,3] #adjusted to 4 to allow for extreme sources as in fig. 24 of Callingham+2017
        self.retrig_prior_dict[prefix + 'c'] = bilby.prior.Uniform(minimum = 0, maximum = 4)

        #prior for d (alpha_thin) - try uniform [-3, 0] #adjusted to be even with alpha_thick
        self.retrig_prior_dict[prefix + 'd'] = bilby.prior.Uniform(minimum = -4, maximum = 0)

        #prior for Snorm (norm factor on power law)
        self.retrig_prior_dict[prefix + 'S_norm'] = bilby.prior.LogUniform(minimum = 1, maximum = 1e5)

        #prior for alpha (spectral index on power law)
        self.retrig_prior_dict[prefix + 'alpha'] = bilby.prior.Uniform(minimum = -4, maximum = 0)
