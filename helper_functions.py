...
def snellen_sed_func(x, a,b,c, d):
    ...
    d: alpha_thin
    '''
    y = b/(1.-np.exp(-1.))*((x/a)**c)*(1. - np.exp(-(x/a)**(d-c)))
    return y
...
def retriggered_sed_func(x, a, b, c, d, S_norm, alpha):
    ...
    y = b/(1.-np.exp(-1.))*((x/a)**c)* \
         (1. - np.exp(-(x/a)**(d-c))) + S_norm*x**alpha
...
class OrientiModel(george.modeling.Model):
    ...
    def get_value(self, t):
        return 10 ** (self.a + np.log10(t.flatten()) * (self.b + self.c*np.log10(t.flatten())))

class SnellenModel(george.modeling.Model):
    parameter_names = ("a", "b", "c", "d")

    def get_value(self, t):
        return self.b/(1.-np.exp(-1.))*((t.flatten()/self.a)**self.c)*(1. - np.exp(-(t.flatten()/self.a)**(self.d-self.c)))

class RetriggeredModel(george.modeling.Model):
    parameter_names = ("a", "b", "c", "d", "S_norm", "alpha")

    def get_value(self, t):
        return self.b/(1.-np.exp(-1.))*((t.flatten()/self.a)**self.c)*(1. - np.exp(-(t.flatten()/self.a)**(self.d-self.c))) + self.S_norm*t.flatten()**self.alpha
...
def get_param_strs_toplot(model):
    ...
    if 'linear' in model.model_type:
        ...
        c_str = " S$_{{norm}}= {:.2f}^{{+{:.2f}}}_{{-{:.2f}}}$".format(fit_param_intervals[0].median, fit_param_intervals[0].plus, fit_param_intervals[0].minus)
        alpha_str = "$\\alpha = {:.2f}^{{+{:.2f}}}_{{-{:.2f}}}$".format(fit_param_intervals[1].median, fit_param_intervals[1].plus, fit_param_intervals[1].minus)
    ...
    #orienti - put this back in!!
        ...
        if not model.param_intervals:
            raise AttributeError("{} is the best model but its intervals have not been calculated yet, please call \"get_orienti_intervals(model_object)\" before plotting".format(model.model_type))
        #prior order is a, b, c
        peak_freq_str = "$\\nu_{{peak}} = {:.2f}^{{+{:.2f}}}_{{-{:.2f}}}$ MHz".format(model.param_intervals['peak_freq_interval'][0], \
            model.param_intervals['peak_freq_interval'][2], model.param_intervals['peak_freq_interval'][1])
        peak_flux_str = "$S_{{peak}} = {:.2f}^{{+{:.2f}}}_{{-{:.2f}}}$ Jy".format(model.param_intervals['peak_flux_interval'][0], \
            model.param_intervals['peak_flux_interval'][2], model.param_intervals['peak_flux_interval'][1])
        alpha_thick_str = "$\\alpha_{{thick}} = {:.2f}^{{+{:.2f}}}_{{-{:.2f}}}$".format(model.param_intervals['alpha_thick_interval'][0], \
            model.param_intervals['alpha_thick_interval'][2], model.param_intervals['alpha_thick_interval'][1])
        alpha_thin_str = "$\\alpha_{{thin}} = {:.2f}^{{+{:.2f}}}_{{-{:.2f}}}$".format(model.param_intervals['alpha_thin_interval'][0], \
            model.param_intervals['alpha_thin_interval'][2], model.param_intervals['alpha_thin_interval'][1])
    ...
    #snellen
        ...
        peak_freq_str = "$\\nu_{{peak}} = {:.2f}^{{+{:.2f}}}_{{-{:.2f}}}$ MHz".format(fit_param_intervals[0].median, fit_param_intervals[0].plus, fit_param_intervals[0].minus)
        peak_flux_str = "$S_{{peak}} = {:.2f}^{{+{:.2f}}}_{{-{:.2f}}}$ Jy".format(fit_param_intervals[1].median, fit_param_intervals[1].plus, fit_param_intervals[1].minus)
        alpha_thick_str = "$\\alpha_{{thick}} = {:.2f}^{{+{:.2f}}}_{{-{:.2f}}}$".format(fit_param_intervals[2].median, fit_param_intervals[2].plus, fit_param_intervals[2].minus)
        alpha_thin_str = "$\\alpha_{{thin}} = {:.2f}^{{+{:.2f}}}_{{-{:.2f}}}$".format(fit_param_intervals[3].median, fit_param_intervals[3].plus, fit_param_intervals[3].minus)
        plot_str_list = [peak_freq_str, peak_flux_str, alpha_thick_str, alpha_thin_str]
    ...
    #retriggered
        ...
        if not model.param_intervals:
            raise AttributeError("{} is the best model but its intervals have not been calculated yet, please call \"get_retrig_intervals(model_object)\" before plotting".format(model.model_type))
        ...
        peak_flux_str = "$S_{{peak}} = {:.2f}^{{+{:.2f}}}_{{-{:.2f}}}$ Jy".format(model.param_intervals['peak_flux_interval'][0], \
            model.param_intervals['peak_flux_interval'][2], model.param_intervals['peak_flux_interval'][1])
        trough_freq_str = "$\\nu_{{min}} = {:.2f}^{{+{:.2f}}}_{{-{:.2f}}}$ MHz".format(model.param_intervals['trough_freq_interval'][0], \
            model.param_intervals['trough_freq_interval'][2], model.param_intervals['trough_freq_interval'][1])
        trough_flux_str = "$S_{{min}} = {:.2f}^{{+{:.2f}}}_{{-{:.2f}}}$ Jy".format(model.param_intervals['trough_flux_interval'][0], \
            model.param_intervals['trough_flux_interval'][2], model.param_intervals['trough_flux_interval'][1])
        alpha_retrig_str = "$\\alpha_{{retrig}} = {:.2f}^{{+{:.2f}}}_{{-{:.2f}}}$".format(model.param_intervals['alpha_retrig_interval'][0], \
            model.param_intervals['alpha_retrig_interval'][2], model.param_intervals['alpha_retrig_interval'][1])
        alpha_thick_str = "$\\alpha_{{thick}} = {:.2f}^{{+{:.2f}}}_{{-{:.2f}}}$".format(model.param_intervals['alpha_thick_interval'][0], \
            model.param_intervals['alpha_thick_interval'][2], model.param_intervals['alpha_thick_interval'][1])
        alpha_thin_str = "$\\alpha_{{thin}} = {:.2f}^{{+{:.2f}}}_{{-{:.2f}}}$".format(model.param_intervals['alpha_thin_interval'][0], \
            model.param_intervals['alpha_thin_interval'][2], model.param_intervals['alpha_thin_interval'][1])
        plot_str_list = [peak_freq_str, peak_flux_str, trough_freq_str, trough_flux_str, alpha_retrig_str, alpha_thick_str, alpha_thin_str]

    else:
        raise TypeError('I cannot find this type of model to extract parameter strings. (Yikes!)')
...
def get_best_model(array):
    ...
    if bf1 > 0 and (md_array[0] - md_array[1]) > 0: #make sure that the second model is simpler
        if bf1 < 0.5:# and bmd_array[1] < bmd_array[0]: #np.log10(md_array[0] - md_array[1] + 1): 
    ...
    if log10Z_arr.shape[0] > 2:
        ...
        if bf2 > 0 and (md_array[0] - md_array[2]) > 0: #make sure that the third model is simpler
            if bf2 < 0.5:# and bmd_array[2] < bmd_array[0]: #np.log10(md_array[0] - md_array[2] + 1):
...
def get_orienti_intervals(result, SED_func, gp = False, freq_min = 1e7, freq_max = 1e11, bilby = True, min_obs_freq = np.nan, max_obs_freq = np.nan):
    '''
    ...
    freq_arr = 10 ** (np.linspace(np.log10(freq_min/1e6), np.log10(freq_max/1e6), 1000) )#np.linspace(np.log10(70), np.log10(30e3)
    ...
    return freq_credible_interval, flux_credible_interval, alpha_thick_interval, alpha_thin_interval

def get_retrig_intervals(result, SED_func, gp = False, freq_min = 1e7, freq_max = 1e11, bilby = True, min_obs_freq = np.nan, max_obs_freq = np.nan):
    '''
    gets the confidence intervals for:
    the peak frequency and corresponding flux
    ...
    flux_dist = SED_func(freq_arr.reshape(-1,1), a_result, b_result, c_result, d_result, s_norm_result, alpha_result)
    flux_dist = flux_dist.transpose()
    ...
    max_lower_idx = np.argmin(np.abs(freq_arr - peak_freq_interval[0]*(1-distance_factor)))
    max_upper_idx = np.argmin(np.abs(freq_arr - peak_freq_interval[0]*(1+distance_factor)))
    ...
    min_lower_idx = np.argmin(np.abs(freq_arr - trough_freq_interval[0]*(1-distance_factor)))
    min_upper_idx = np.argmin(np.abs(freq_arr - trough_freq_interval[0]*(1+distance_factor)))
    ...
    print('calculating indices at: {:.2f}, {:.2f}, {:.2f}, {:.2f} MHz'.format(freq_arr[max_lower_idx], \
        freq_arr[max_upper_idx], freq_arr[min_lower_idx], freq_arr[min_upper_idx]))
    
    #print(trough_freq_interval[0], trough_freq_interval[0]*(1-distance_factor), freq_arr)
    

    #peak above trough
    if peak_freq_interval[0] > trough_freq_interval[0] and peak_freq_interval[0] > freq_min/1e6 and trough_freq_interval[0] > freq_min/1e6:
        func_type = 'minfirst'
        ...
        alphathickgrads = (np.log10(flux_dist[:, max_lower_idx]) - np.log10(flux_dist[:, min_upper_idx]))/(np.log10(freq_arr_broadcast[:, max_lower_idx]) - np.log10(freq_arr_broadcast[:, min_upper_idx]))
        ...
        retriggrads = (np.log10(flux_dist[:, min_lower_idx]) - np.log10(flux_dist[:,0]))/(np.log10(freq_arr_broadcast[:, min_lower_idx]) - np.log10(freq_arr_broadcast[:,0]))
        ...
        thingrads = (np.log10(flux_dist[:,-1]) - np.log10(flux_dist[:, max_upper_idx]))/(np.log10(freq_arr_broadcast[:,-1]) - np.log10(freq_arr_broadcast[:, max_upper_idx]))

        #get the credible interval based on this
        alpha_thin_interval = get_credible_interval(thingrads)

    elif peak_freq_interval[0] < trough_freq_interval[0]  and peak_freq_interval[0] > 0 and trough_freq_interval[0] > 0:
        func_type = 'maxfirst'
        ...
        alphathickgrads = (np.log10(flux_dist[:, max_lower_idx]) - np.log10(flux_dist[:,0]))/(np.log10(freq_arr_broadcast[:, max_lower_idx]) - np.log10(freq_arr_broadcast[:,0]))
        ...
        retriggrads = (np.log10(flux_dist[:,-1]) - np.log10(flux_dist[:, min_upper_idx]))/(np.log10(freq_arr_broadcast[:,-1]) - np.log10(freq_arr_broadcast[:, min_upper_idx]))
       ...
       #thingrads = (np.log10(flux_dist[local_min_idxs]) - np.log10(flux_dist[local_max_idxs]))/(np.log10(freq_arr_broadcast[local_min_idxs]) - np.log10(freq_arr_broadcast[local_max_idxs]))
        thingrads = (np.log10(flux_dist[:, max_upper_idx]) - np.log10(flux_dist[:, min_lower_idx]))/(np.log10(freq_arr_broadcast[:, max_upper_idx]) - np.log10(freq_arr_broadcast[:, min_lower_idx]))
    ...
    #we have only a trough and no peak
        ...
        alphathickgrads = (np.log10(flux_dist[:,-1]) - np.log10(flux_dist[:, min_upper_idx]))/(np.log10(freq_arr_broadcast[:,-1]) - np.log10(freq_arr_broadcast[:, min_upper_idx]))
    ...
    #we have only a peak and no trough - shouldn't happen we should pick SNELLEN
        ...
        func_type = 'linear'
        retriggrads = (np.log10(flux_dist[:,-1]) - np.log10(flux_dist[:,0]))/(np.log10(freq_arr_broadcast[:,-1]) - np.log10(freq_arr_broadcast[:,0]))
    ...
    return peak_freq_interval, peak_flux_interval, trough_freq_interval, trough_flux_interval, alpha_retrig_interval, alpha_thick_interval, alpha_thin_interval, func_type
...
def get_neighbour_density(survey_list, ra, dec):
    ...
    cat_data = pd.read_csv(filepath2 + fname, sep = ',', header = 0, skiprows = list(range(26,58)))

    cat_data = cat_data[cat_data['bibcode'].isin(survey_list)]
    source_beams = cat_data['max beam size (arcsec)'].apply(lambda x: float(x.split(';')[0]))
    #sort values in cat
    ...
    near_isls = isl_data[(isl_data['ra'] - ra)**2 + (isl_data['dec'] - dec)** 2 <= max_beam/(60*60)]
...
def check_retrig_is_peaked(result, SED_func, gp = False, freq_min = 1e7, freq_max = 1e11, bilby = True, peak_freq = np.nan, trough_freq = np.nan):
...
class GaussianCovLikelihood(bilby.Likelihood):
    ...
    def log_likelihood(self):
        ...
        residuals = self.y - self.function(self.x, **model_parameters) # ydata - SED_func(xdata)
        return -0.5*np.dot(residuals.T, cho_solve((sigma, False), residuals)) # -0.5*tranpose(R)*Inverse(covariance matrix)*R
