...
def check_gleam_compactness(ra,dec, gleam_viz = gleam_vizier, radius = 25):
    ...
    print(res)
    try:
        res = Vizier.query_region(coords, radius = radius*u.arcsec, catalog=gleam_vizier)[0].to_pandas()
    except IndexError:
        return -1, -1, -1
    print(res.columns)
    ...
    at20g_coord = Coord.SkyCoord(ra = res['RAJ2000'], dec = res['DEJ2000'], unit = (u.hourangle, u.deg))
...
def check_at20g_compactness(ra, dec, at20g_viz = at20g_vizier, radius = 19):
    ...
    coords = Coord.SkyCoord(ra=ra, dec=dec,unit=(u.deg, u.deg),frame='icrs')
    try:
        res = Vizier.query_region(coords, radius = radius*u.arcsec, catalog=at20g_viz)[0].to_pandas()
    except IndexError:
        return -1, -1, -1
    ...
    is_compact = visibility > 0.86 #from Chhetri et al. 2012, MNRAS 422, 2274 (section 3)
...
def resolve_name_generic(iau_name, racs_viz = racs_vizier):
    ...
    racs_coord = Coord.SkyCoord(ra = res['RAJ2000'].values[0]*u.deg, dec = res['DEJ2000'].values[0]*u.deg)
    separation = racs_coord.separation(result)

    if separation.deg*60*60 > 60:
        raise IndexError('No RACS source found within 1 arcminute of {}'.format(iau_name))
...
def racs_id_to_name(racs_id):
    ...
    res = Vizier.query_constraints(catalog = racs_vizier, ID= '={}'.format(racs_id))[0].to_pandas()
...
if __name__ == '__main__':
    ...
    res = Vizier.query_region('RACS-DR1 J200826.5+001049', radius = 1*u.arcmin, catalog=racs_vizier)
